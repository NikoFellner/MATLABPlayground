function plot_Fachwerk(konn, num_knoten, F, num_forces, koord, num_stab, lager, num_lager, color, linewidth, linewidth_scale)
x = koord(:,1);
y = koord(:,2);
hold on
%Beschriftung und plotten der Stäbe
for i=1:num_stab
    if ~isempty(linewidth_scale)
        linewidth_scaled = linewidth * linewidth_scale(i);
        p1 = plot([x(konn(i,1)) x(konn(i,2))], [y(konn(i,1)) y(konn(i,2))], 'linewidth',linewidth_scaled, 'Color',color);
    else
        p1 = plot([x(konn(i,1)) x(konn(i,2))], [y(konn(i,1)) y(konn(i,2))], 'linewidth',linewidth, 'Color',color);
    end
